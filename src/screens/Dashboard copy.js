 ...
 const [isDeveloperSetting, setIsDeveloperSetting] = useState(false);
  ...
  const handleSwitchLogin = useCallback(async () => {
    ...
    try {
      ...
      setIsContact(is_contact === true || is_contact === 'true');
      setSwitchPassword('');
      setSwitchPin('');
      setSwitchError('');
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      console.log(enableDeveloper, 'enableDeveloper');
        ...
        const storedIsContact = await AsyncStorage.getItem('is_contact');
        ...
        setIsContact(storedIsContact === 'true');
      } catch (e) {
        setUserName('');
        setUserEmail('');
        setUserRole('');
  ...
  const visibleCards = useMemo(() => {
    return cards.filter((card) => {
      ...
      // Only show Contact Us card if user has permission
      if (card.key === 'support-team') {
        return isContact;
      }
      return true;
    });
