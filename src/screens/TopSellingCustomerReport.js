...
import { exportTopSellingCustomersToExcel } from '../functions/reports/exportReportsExcel';
...
export default function TopSellingCustomerReport() {
  ...
  const handleDownloadReport = async () => {
    ...
    setDownloading(true);
    try {
      await exportTopSellingCustomersToExcel({
        rows: rows.map((item) => ({
          name: getCustomerName(item),
          phone: getPhone(item),
          saleAmount: getSalesAmount(item),
        })),
        totalSales,
        dateRangeLabel: `${fmtDateOnly(range.start)} to ${fmtDateOnly(range.end)}`,
        fileName: `Top_Selling_Customers_${fmtDateOnly(range.start)}_to_${fmtDateOnly(range.end)}.xlsx`,
      });
    } catch (e) {
      console.warn('Top selling customers export failed:', e);
