...
export default function ChatScreen({ route, navigation }) {
  ...
  useEffect(() => {
    (async () => {
      await AsyncStorage.multiGet(['callUserEmail', 'callUserName']).then((pairs) => {
   ...
   useEffect(() => {
    ...
    (async () => {
      ...
      const agentList = storeDoc.data()?.Agent || [];
       ...
       setIsAgent(agent);
      console.log("userrole:",isAgent,'agent:',agent, 'myEmail:',myEmail);
...
const styles = StyleSheet.create({
  ...
  bubbleSender: { fontSize: 11, fontWeight: '700', color: '#319241', marginBottom: 3 },
