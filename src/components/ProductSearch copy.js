  ...
  try {
    ...
    for (const attempt of attempts) {
      ...
      if (Array.isArray(result?.products) && result.products.length > 0) {
        ...
        // Check if first product has variants array
        if (Array.isArray(firstProduct?.variants) && firstProduct.variants.length > 0) {
          // Filter variants to show only exact barcode match
          const exactMatches = firstProduct.variants.filter(v => 
            String(v?.barcode || "").trim() === originalBarcode
          );
          ...
          if (exactMatches.length > 0) {
            ...
            matchedBarcode = originalBarcode;
            break;
          }
        } else {
          // Filter regular products to show only exact barcode match
          const exactMatches = result.products.filter(p => 
            String(p?.barcode || "").trim() === originalBarcode
          );
